...
public class ServerBeanContext {
	...
	private static final Map<String, VersionWrapper> beanVersionMap = new HashMap<>();
	...
	{
		VersionWrapper wrap = beanVersionMap.get(version);
		if(wrap == null)
		{
			//TODO 
		}
...
//	}
	...
	private static synchronized void generate(VersionBean v) {

		Class<?> beanCla = v.getBean().getClass();

		VersionWrapper vWrap = new VersionWrapper(v.getVersion());
		beanVersionMap.put(v.getVersion(), vWrap);
		
		
		BeanWrapper wrap = vWrap.initVersionBean(beanSeq, v.getInterfaceName(), v.getBean());
		beanSeq++;
		...
		for (Method method : methods) {
			...
			wrap.putMethodBean(methodSeq, new MethodWrapper(methodSeq, methodName, method));
	...
	private static void pushToRegistry() throws Exception{
		...
		for(Entry<String, VersionWrapper> entry:beanVersionMap.entrySet())
		{
			String version = entry.getKey();
			VersionWrapper vBeanWrap = entry.getValue();
